...
function getBookingDates() {
  ...
  checkInDate2.setDate(checkInDate.getDate() + 2);
  const checkInDate3 = new Date(checkInDate);
  checkInDate3.setDate(checkInDate.getDate() + 3);
  // สร้างตัวแปรสำหรับ Check-out และบวกเพิ่มไป 4 วัน
  ...
  return {
    ...
    checkIn1: formatDate(checkInDate1),
    checkIn2: formatDate(checkInDate2),
    checkIn3: formatDate(checkInDate3),
...
function generateRooms(): RoomRecord[] {
  ...
  const types = [
    { type: 'Superior', prefix: 'S', start: 101, count: 100 },
    { type: 'Deluxe', prefix: 'D', start: 201, count: 100 },
    { type: 'Suite', prefix: 'T', start: 301, count: 100 },
  ];
...
const INITIAL_PENDING_BOOKINGS: PendingBooking[] = [
  { id: 'pb1', guestName: 'Amanda White', roomType: 'Superior', checkIn: getBookingDates().checkIn, checkOut: getBookingDates().checkOut, confirmationNumber: 'KU-P001', totalAmount: 2400 },
  { id: 'pb2', guestName: 'David Lee', roomType: 'Superior', checkIn: getBookingDates().checkIn1, checkOut: getBookingDates().checkOut, confirmationNumber: 'KU-P002', totalAmount: 4000 },
  { id: 'pb3', guestName: 'Maria Garcia', roomType: 'Deluxe', checkIn: getBookingDates().checkIn2, checkOut: getBookingDates().checkOut, confirmationNumber: 'KU-P003', totalAmount: 1800 },
  { id: 'pb4', guestName: 'Somsak Pol', roomType: 'Suite', checkIn: getBookingDates().checkIn3, checkOut: getBookingDates().checkOut, confirmationNumber: 'KU-P004', totalAmount: 7000 },
];
...
function FilterPill({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all whitespace-nowrap ${active
...
export function AdminDashboard() {
  ...
  const filteredPendingBookings = useMemo(() => {
    if (!bookingDateFilter) return pendingBookings;
    return pendingBookings.filter(b => b.checkIn === bookingDateFilter);
  }, [pendingBookings, bookingDateFilter]);
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                จัดการห้องพักทั้งหมด {rooms.length} ห้อง · มีรายการรอ Assign {filteredPendingBookings.length} รายการ
                {bookingDateFilter && <span className="text-[#006b54] font-bold ml-1">(วันที่ {bookingDateFilter})</span>}
              </p>
            </div>
            <div className="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
              ...
              <div className="relative w-full md:w-44">
                ...
                <Input
                  type="date"
                  className="pl-9 text-sm focus:ring-[#006b54]"
                  value={bookingDateFilter}
                  onChange={(e) => setBookingDateFilter(e.target.value)}
                  title="กรองวันเข้าพักของรายการรอ Assign"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
            ...
            <button
              ...
              className={`rounded-xl px-3 py-2 text-center border transition-all ${statusFilter === 'all'
                ? 'bg-[#006b54] text-white border-[#006b54]'
            ...
            {(Object.keys(STATUS_CONFIG) as RoomStatus[]).map((s) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl px-3 py-2 text-center border-2 transition-all ${active ? c.bg : 'bg-white border-gray-200 hover:border-gray-300'
                    }`}
                >
      ...
      <div className="container mx-auto px-4 py-5">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-5">
          ...
          {(search || statusFilter !== 'all' || typeFilter !== 'all' || bookingDateFilter) && (
            <button
              onClick={() => { setSearch(''); setStatusFilter('all'); setTypeFilter('all'); setBookingDateFilter(''); }}
        ...
        {byFloor.length === 0 ? (
          ...
          <div className="space-y-4">
            {byFloor.map(([floor, floorRooms]) => {
              ...
              return (
                <div key={floor} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                  {/* Floor header */}
                  <button
