...
export function SuiteDetailPage() {
  // Currently hardcoded to 'suite' room based on URL pattern in App.tsx
  // In a real app, this would use useParams to get the room ID
  const room = ROOMS.find(r => r.id === 'suite') || ROOMS[2];
  ...
  const getAmenityIcon = (amenity: string) => {
    ...
    if (lower.includes('fridge') || lower.includes('refrigerator')) return <Refrigerator className="w-5 h-5" />;
  ...
  const totalPrice = room.rates.daily.general + (extraBeds * room.extraBedPrice);
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-2xl font-bold text-[#006b54]">
                      {(room.rates.daily.personnel + (extraBeds * room.extraBedPrice)).toLocaleString()} THB
                    </div>
                  </div>
                </div>
